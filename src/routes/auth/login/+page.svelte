...
<div class="h-full flex justify-center items-center">
	<div class="card max-w-lg w-full p-3 sm:p-6 variant-ringed space-y-4">
			...
			{#if showResetPassword}
				...
				<div class="flex justify-between">
					<button
						...
						class="btn min-w-[100px] capitalize variant-ghost-primary"
					>
						{#if loading}
					...
					<button
						...
						type="submit"
						class="btn min-w-[100px] capitalize variant-ghost-surface">{$t('common.cancel')}</button
					>
				</div>
			{:else}
				...
				<form method="POST" action="?/login" use:enhance={handleLogin}>
					...
					<div class="flex justify-between pt-4">
						...
						<button
							disabled={loading}
							type="submit"
							class="btn min-w-[100px] variant-ghost-primary"
						>
							{#if loading}
								<ProgressRadial width="w-6" />
							{:else}
